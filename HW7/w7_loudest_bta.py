...
def loudest_band(music,frame_rate,bandwidth):
	...
	this_sum = np.sum(fft_power[zero_Hz_index-1:zero_Hz_index+window_width])

	for i in range(zero_Hz_index, N-window_width): #Loop over positive frequencies
		#this_sum = np.sum(fft_power[i:i+window_width])
		this_sum = this_sum - fft_power[i-1] + fft_power[i+window_width-1]
		if this_sum > max_sum:
			max_sum   = this_sum
			max_sum_i = i
	...
	for i in range(max_sum_i,max_sum_i+window_width+1):
		H[i] = 1
	for i in range(2*zero_Hz_index-max_sum_i-window_width,2*zero_Hz_index-max_sum_i+1):
		H[i] = 1
