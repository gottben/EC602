...
int_matrix multiply_int(int_matrix A, int_matrix B);
double_matrix multiply_double(double_matrix A, double_matrix B);
...
int convert_double(string str_number, double double_number)
{
	...
	double_number = atof(str_number.c_str());
	return 0;
}
...
int read_in_double(string file, double_matrix &matrix, int M, int N)
{
	...
	cout << elements.size() << "M=" << M << " and N=" << N << endl;
	thisfile.close();
